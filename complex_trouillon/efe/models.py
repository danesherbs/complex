...
class Abstract_Model(object):
    ...
    def get_loss_args_and_symb_vars(self, train_triples, valid_triples,
                                    hparams):
        """
        ...
        train = Batch_Loader(train_triples, n_entities=max(self.n, self.l),
                             batch_size=hparams.batch_size,
                             neg_ratio=hparams.neg_ratio,
                             contiguous_sampling=hparams.contiguous_sampling)
        inputs = [self.ys, self.rows, self.cols, self.tubes]
        if valid_triples is not None:
            valid = Batch_Loader(valid_triples, n_entities=max(self.n, self.l),
                                 batch_size=hparams.batch_size,
                                 neg_ratio=hparams.neg_ratio,
                                 contiguous_sampling=hparams.contiguous_sampling)
    ...
    def setup_params_for_train(self, train_triples, valid_triples, hparams,
                               redefine_loss=False):
        """
        ...
        if redefine_loss or self.loss_to_opt is None:
            ...
            self.pred_func_compiled = theano.function(
                self.get_pred_symb_vars(), self.pred_func)
    ...
    def fit(self, train_triples, valid_triples, hparams, n=0, m=0, l=0,
            scorer=None):
        ...
        # get the loss inputs:
        train_vals, train_symbs, valid_vals = self.get_loss_args_and_symb_vars(
            train_triples, valid_triples, hparams)

        opt = downhill.build(hparams.learning_rate_policy,
                             loss=self.loss_to_opt, inputs=train_symbs,
        ...
        for tm, vm in opt.iterate(train_vals, None,
                                  ...
                                  # I take care of the valiation, with validation metrics instead of loss
                                  patience=9999999,
                                  # Number of tolerated imporvements of validation loss that are inferior to min_improvement
                                  max_gradient_norm=1,
            ...
            if it % hparams.valid_scores_every == 0 and scorer is not None:
                if valid_triples is not None:
                    ...
                    res = scorer.compute_scores(self, self.name, hparams,
                                                valid_triples)
                    cv_res = CV_Results()
                    cv_res.add_res(res, self.name, hparams.embedding_size,
                                   hparams.lmbda, self.nb_params)
                    ...
                    if scorer.compute_ranking_scores:
                        ...
                        if best_valid_mrr >= metrics[self.name][2]:
                            logger.info(
                                "Validation filtered MRR decreased, stopping here.")
                            break
                        else:
                        ...
                        if best_valid_ap >= res.ap:
                            logger.info(
                                "Validation AP decreased, stopping here.")
                            break
                        else:
...
class CP_Model(Abstract_Model):
    ...
    def define_loss(self):
        ...
        self.pred_func = TT.sum(
            self.u[self.rows, :] * self.v[self.cols, :] * self.w[self.tubes,
                                                          :], 1)
...
class CP_Logistic_Model(CP_Model):
    ...
    def define_loss(self):
        # Defining the loss by decomposing each part:
        self.pred_func = TT.nnet.sigmoid(TT.sum(
            self.u[self.rows, :] * self.v[self.cols, :] * self.w[self.tubes,
                                                          :], 1))

        self.loss = TT.nnet.softplus(- self.ys * TT.sum(
            self.u[self.rows, :] * self.v[self.cols, :] * self.w[self.tubes,
                                                          :], 1)).mean()
...
class TransE_L2_Model(Abstract_Model):
    ...
    def get_init_params(self):
        ...
        params = {'e': randn(max(self.n, self.l), self.k),
                  # np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (max(self.n,self.l),self.k)).astype('f')
                  'r': L2_proj(randn(self.m, self.k)),
                  # L2_proj(np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (self.m,self.k))).astype('f')
                  }
        return params
    ...
    def get_loss_args_and_symb_vars(self, train_triples, valid_triples,
        ...
        train = TransE_Batch_Loader(self, train_triples,
                                    n_entities=max(self.n, self.l),
                                    batch_size=hparams.batch_size,
                                    neg_ratio=hparams.neg_ratio,
                                    contiguous_sampling=hparams.contiguous_sampling)
    ...
    def setup_params_for_train(self, train_triples, valid_triples, hparams,
        ...
        super(TransE_L2_Model, self).setup_params_for_train(train_triples,
                                                            valid_triples,
                                                            hparams,
                                                            redefine_loss)
    ...
    def define_loss(self):
        ...
        self.pred_func = - TT.sqrt(TT.sum(TT.sqr(
            self.e[self.rows, :] + self.r[self.cols, :] - self.e[self.tubes,
                                                          :]), 1))

        self.loss = TT.maximum(0, self.margin + TT.sqrt(TT.sum(TT.sqr(
            self.e[self.rows[:self.batch_size], :] + self.r[self.cols[
                                                            :self.batch_size],
                                                     :] - self.e[self.tubes[
                                                                 :self.batch_size],
                                                          :]), 1)) \
                               - (1.0 / self.neg_ratio) * TT.sum(TT.sqrt(
            TT.sum(TT.sqr(self.e[self.rows[self.batch_size:], :] + self.r[
                                                                   self.cols[
                                                                   self.batch_size:],
                                                                   :] - self.e[
                                                                        self.tubes[
                                                                        self.batch_size:],
                                                                        :]),
                   1)).reshape((int(self.batch_size), int(self.neg_ratio))),
                                                                 1)).mean()
...
class TransE_L1_Model(TransE_L2_Model):
    ...
    def setup_params_for_train(self, train_triples, valid_triples, hparams,
                               redefine_loss=False):
        super(TransE_L1_Model, self).setup_params_for_train(train_triples,
                                                            valid_triples,
                                                            hparams,
                                                            redefine_loss)

    def define_loss(self):
        self.pred_func = - TT.sum(TT.abs_(
            self.e[self.rows, :] + self.r[self.cols, :] - self.e[self.tubes,
                                                          :]), 1)

        self.loss = TT.maximum(0, self.margin + TT.sum(TT.abs_(
            self.e[self.rows[:self.batch_size], :] + self.r[self.cols[
                                                            :self.batch_size],
                                                     :] - self.e[self.tubes[
                                                                 :self.batch_size],
                                                          :]), 1) \
                               - (1.0 / self.neg_ratio) * TT.sum(TT.sum(
            TT.abs_(self.e[self.rows[self.batch_size:], :] + self.r[self.cols[
                                                                    self.batch_size:],
                                                             :] - self.e[
                                                                  self.tubes[
                                                                  self.batch_size:],
                                                                  :]),
            1).reshape((int(self.batch_size), int(self.neg_ratio))), 1)).mean()
...
class Complex_Model(Abstract_Model):
    ...
    def define_loss(self):
        self.pred_func = TT.sum(
            self.e1[self.rows, :] * self.r1[self.cols, :] * self.e1[self.tubes,
                                                            :], 1) \
                         + TT.sum(
            self.e2[self.rows, :] * self.r1[self.cols, :] * self.e2[self.tubes,
                                                            :], 1) \
                         + TT.sum(
            self.e1[self.rows, :] * self.r2[self.cols, :] * self.e2[self.tubes,
                                                            :], 1) \
                         - TT.sum(
            self.e2[self.rows, :] * self.r2[self.cols, :] * self.e1[self.tubes,
                                                            :], 1)
    ...
    def define_loss(self):
        ...
        self.loss = TT.nnet.softplus(- self.ys * (TT.sum(
            self.e1[self.rows, :] * self.r1[self.cols, :] * self.e1[self.tubes,
                                                            :], 1) \
                                                  + TT.sum(
                    self.e2[self.rows, :] * self.r1[self.cols, :] * self.e2[
                                                                    self.tubes,
                                                                    :], 1) \
                                                  + TT.sum(
                    self.e1[self.rows, :] * self.r2[self.cols, :] * self.e2[
                                                                    self.tubes,
                                                                    :], 1) \
                                                  - TT.sum(
                    self.e2[self.rows, :] * self.r2[self.cols, :] * self.e1[
                                                                    self.tubes,
                                                                    :],
                    1))).mean()
...
class Rescal_Model(Abstract_Model):
    ...
    def define_loss(self):
        self.pred_func = TT.sum(
            TT.sum(self.e[self.rows, :, None] * self.r[self.cols, :, :],
                   1) * self.e[self.tubes, :], 1)
