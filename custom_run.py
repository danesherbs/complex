...
if __name__ == "__main__":
    ...
    fb15kexp = build_data(name='custom_data', path=tools.cur_path + '/datasets/')

    # SGD hyper-parameters:
    params = Parameters(learning_rate=0.5,
                        max_iter=1000,
                        batch_size=int(len(fb15kexp.train.values) / 100),
                        # Make 100 batches
                        neg_ratio=10,
    ...
    fb15kexp.grid_search_on_all_models(all_params,
                                       embedding_size_grid=[emb_size],
                                       lmbda_grid=[lmbda], nb_runs=1)
